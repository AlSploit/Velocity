...
task.spawn(function()
	local DamageFly = CreateToggle("DamageFly", BlatantTab, Settings.DamageFly.Value, function(CallBack)
		Settings.DamageFly.Value = CallBack
	end)
end)
...
task.spawn(function()
	local FastConsume = CreateToggle("FastConsume", UtilityTab, Settings.FastConsume.Value, function(CallBack)
		Settings.FastConsume.Value = CallBack
	end)
end)
...
task.spawn(function()
	local FovChanger, DropDownButton, LayoutOrder = CreateToggle("FovChanger", WorldTab, Settings.FovChanger.Value, function(CallBack)
		Settings.FovChanger.Value = CallBack
		
		local OldSetFov = FovController.setFOV
		local OldGetFov = FovController.getFOV
		
		if CallBack == true then
			FovController.setFOV = function(self) 
				return OldSetFov(self, Settings.FovChanger.Fov) 
			end
			
			FovController.getFOV = function(...) 
				return Settings.FovChanger.Value
			end
		end

		if CallBack == false then
			FovController.setFOV = OldSetFov
			FovController.getFOV = OldGetFov
		end
	end)

	local FovChangerValue = true
	local FovChanger

	DropDownButton.Activated:Connect(function()
		if FovChangerValue == true then
		...
		if FovChangerValue == false then
			FovChanger:Destroy()
		end

		FovChangerValue = not FovChangerValue
	end)
end)
...
task.spawn(function()
	...
	local function GetRemote(Remote)
	...
	local PickupRemote = GetRemote(debug.getconstants(KnitClient.Controllers.ItemDropController.checkForPickup))
